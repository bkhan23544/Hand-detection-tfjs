...
function App() {
  ...
  const [backend, setBackend] = React.useState('webgl')
  ...
  const loadCustomModel = async () => {
    tf.setBackend('webgl').then(() => console.log("backend is " + tf.getBackend()))
    addFlagLables()
  ...
  const detectObjects = async (models) => {
    ...
    if (models) {
      ...
      const smallImg = tf.image.resizeBilinear(tfImg, [80, 80]) // 600, 450
      const resized = tf.cast(smallImg, 'int32')
      const tf4d = tf.tensor4d(Array.from(resized.dataSync()), [1, 80, 80, 3])
